...
export class ReservationRequestOldComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    //console.log('im in reservation add user: ', this.user.id + " & " + this.user.username);
        ...
        this.rs.listOfReservationsByUserId(+this.user.id).subscribe((resList: Reservation[]) => {
          this.reservationArr = resList;
        });
        console.log("this is the length of this.reservationArr: " + this.reservationArr);
